...
public class NettyChannelHandler extends ChannelDuplexHandler implements StatisticCallback {
  ...
  public Codec codec;
  public AtomicInteger rejectCounter=new AtomicInteger(0);
  ...
  public void rejectMessage(  ChannelHandlerContext ctx,  NettyMessage msg){
    if (msg.isRequest()) {
      ...
      LoggerUtil.error("process thread pool is full, reject, active={} poolSize={} corePoolSize={} maxPoolSize={} taskCount={} requestId={}",threadPoolExecutor.getActiveCount(),threadPoolExecutor.getPoolSize(),threadPoolExecutor.getCorePoolSize(),threadPoolExecutor.getMaximumPoolSize(),threadPoolExecutor.getTaskCount(),msg.getRequestId());
      rejectCounter.incrementAndGet();
    }
  }
  public void processMessage(  ChannelHandlerContext ctx,  NettyMessage msg){
  ...
  public Response buildExceptionResponse(  long requestId,  Exception e){
    DefaultResponse response=new DefaultResponse();
    response.setRequestId(requestId);
    response.setException(e);
    return response;
  }
  public void processRequest(  final ChannelHandlerContext ctx,  final Request request){
  ...
  public ChannelFuture sendResponse(  ChannelHandlerContext ctx,  Response response){
  ...
  @Override public String statisticCallback(){
